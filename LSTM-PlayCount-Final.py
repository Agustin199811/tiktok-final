...
file_path = 'procesados/playC_transformado_digg_playcount.csv'
...
# Eliminar outliers
q1 = data['playCount'].quantile(0.25)
q3 = data['playCount'].quantile(0.75)
iqr = q3 - q1
data = data[(data['playCount'] >= (q1 - 1.5 * iqr)) & (data['playCount'] <= (q3 + 1.5 * iqr))]
...
# Verificar el número de filas antes de eliminar NaN
print(f"Rows before dropna: {data.shape[0]}")

# Eliminar las filas con valores NaN generados por el desplazamiento
data.dropna(inplace=True)

# Verificar el número de filas después de eliminar NaN
print(f"Rows after dropna: {data.shape[0]}")
...
model.save('LSTM-playCount.keras', save_format='keras')
...
print("Rango de datos originales (playCount):", y_test.min(), y_test.max())
